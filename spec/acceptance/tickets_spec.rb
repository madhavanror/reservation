...
resource 'Tickets' do
  ...
  post 'api/v1/tickets' do
    parameter :request_number, 'Request Number', required: true
    parameter :request_type, 'Request Type', required: true
    parameter :company_name, 'Company Name', required: true

    let!(:raw_post) { ticket_params.to_json }
    example_request "Log A Ticket" do
      expect(status).to eq(201)
    end
  end

  get 'api/v1/tickets/ticket/:id' do
    parameter :id, 'Ticket Id', required: true
    
    let(:id) { ticket.id }
    example_request 'Get the ticket by id' do
      expect(status).to eq(200)
    end
  end

  get 'api/v1/tickets/show_tickets' do
    example_request 'Get All Tickets' do
      expect(status).to eq(200)
    end
  end
end
